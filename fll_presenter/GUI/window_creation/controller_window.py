...
def new_controller_window(
    orientation, music_options=True, timer_options=True, timer=None, timer_window=None
):
    ...
    if timer_options:
        ...
        toggle_full_screen_button = ct.CTkButton(
            controller,
            text="Toggle\nFull\nScreen",
        ...
        update_color_button = ct.CTkButton(
            controller,
            text="Change\nTimer\nBackground",
            command=lambda: timer_window.configure(
                background=str(shared_functions.open_color_menu(timer_window))
            ),
            width=115,
            height=3,
            font=("Serif", 20),
        ...
        start_timer_button = ct.CTkButton(
            controller,
            text="Start Timer",
            command=lambda: Timer_Element.start_timer(timer),
            width=110,
            height=3,
            font=("Serif", 20),
        )

        stop_timer_button = ct.CTkButton(
            controller,
            text="Stop Timer",
            command=lambda: Timer_Element.stop_timer(timer),
            width=110,
            height=3,
            font=("Serif", 20),
        )

        timer_toggle_button = ct.CTkButton(
            controller, text="Hide/Show Timer", width=110, height=3, font=("Serif", 20)
        )
        timer_toggle_button._command = lambda: shared_functions.toggle_window(
            "Timer Window", timer_toggle_button
        )
        ...
        timer_size = ct.CTkSlider(
            controller, width=20, from_=0, to=1000, orientation="vertical"
        )
        timer_size.configure(command=set_timer_size)
        timer_size.set(200)
        timer_size.after(
            50, lambda: timer_size.pack(side=pack_var, fill="y", expand=True)
        )

    if music_options:
        volume_up_button = ct.CTkButton(
            controller,
            text="Volume Up",
            command=lambda: shared_functions.call_volume_control(
                True, controller, system
            ),
            width=110,
            height=3,
            font=("Serif", 20),
        ...
        volume_down_button = ct.CTkButton(
            controller,
            text="Volume Down",
            command=lambda: shared_functions.call_volume_control(
                False, controller, system
            ),
            width=110,
            height=3,
            font=("Serif", 20),
        ...
        min_volume_slider = ct.CTkSlider(
            controller,
            width=20,
            from_=0,
            to=100,
            ...
            command=shared_functions.set_volume_minimum,
        )
        min_volume_slider.set(vm.get_volume_control()[0])

        max_volume_slider = ct.CTkSlider(
            controller,
            width=20,
            from_=0,
            to=100,
            number_of_steps=100,
            orientation="vertical",
            command=shared_functions.set_volume_maximum,
        )
        ...
        if system == "Windows":
            application_selector = ct.CTkOptionMenu(
                controller,
            ...
            application_selector.after(
                10000,
                lambda: shared_functions.schedule_dropdown_update(
                    controller, application_selector
                ),
            )
