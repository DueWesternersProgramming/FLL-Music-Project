...
def run_gui():
    ...
    global CONFIGWINDOW
    CONFIGWINDOW = ct.CTk()
    CONFIGWINDOW.wm_geometry("800x500")
    ...
    full_gui = ct.CTkButton(
        CONFIGWINDOW,
        text="Full Options",
        command=lambda win=CONFIGWINDOW: open_full_gui(win),
        font=("Serif", 30),
    )
    full_gui.pack(side="top", fill="x")

    timer_only_gui = ct.CTkButton(
        CONFIGWINDOW,
        text="Timer Control Only",
        command=lambda win=CONFIGWINDOW: open_timer_gui(win),
        font=("Serif", 30),
    )
    timer_only_gui.pack(side="top", fill="x")

    music_only_gui = ct.CTkButton(
        CONFIGWINDOW,
        text="Music Control Only",
        command=lambda win=CONFIGWINDOW: open_music_gui(win),
        font=("Serif", 30),
    )
    ...
    CONFIGWINDOW.protocol(
        "WM_DELETE_WINDOW", lambda: shared_functions.kill_windows(CONFIGWINDOW)
    )
