...
def runGUI():
  ...
  saveSendButton = ct.CTkButton(controller, text="Save Data",command=lambda: sheet.update(timerWindow))
  saveSendButton.place(relx=0.5,rely=0.5)
  ...
  startButton.place(relx=0,rely=0.5)
  volumeDownButton.place(relx=0,rely=0.6)
  volumeUpButton.place(relx=0,rely=0.7)

  timer = ct.CTkLabel(timerWindow,text=("2:30"))
  timer.configure(font=("Serif", 180))
  timer.place(relx=.5, rely=.5,anchor="s")

  sheet.initEntrySheet(controller)
  sheet.initDisplaySheet(timerWindow)
